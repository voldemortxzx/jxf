-- Î÷É½Óì Ö°ÄÜ Ìú½³
-- By: li_xin(2005-01-27)
--update:zhaowenyi(2005-02-20)Ôö¼Ó×ÏÉ«¼°»Æ½ğ×°±¸ÖıÔì

Include("\\script\\global\\×ÏÉ«¼°»Æ½ğ×°±¸ÖıÔì.lua")
function main(sel)
	--Say("Muèn sèng ®­îc trªn T©y S¬n Tù µh, kh«ng cã binh khİ lîi h¹i cña ta e r»ng ng­¬i chŞu nhiÒu thiÖt thßi ®ã. Ng­¬i cã muèn mua g× kh«ng? Cßn muèn chÕ t¹o Trang bŞ HuyÒn Tinh hoÆc trang bŞ Hoµng Kim th× cø ®Õn t×m ta gi¸ c¶ ph¶i ch¨ng th«i!", 3, "Giao dŞch/yes", "chÕ t¹o/onFoundry", "Nh©n tiÖn ghĞ qua th«i/no")
	Say("Muèn sèng ®­îc trªn T©y S¬n Tù µh, kh«ng cã binh khİ lîi h¹i cña ta e r»ng ng­¬i chŞu nhiÒu thiÖt thßi ®ã. Ng­¬i cã muèn mua g× kh«ng? Cßn muèn chÕ t¹o Trang bŞ HuyÒn Tinh hoÆc trang bŞ Hoµng Kim th× cø ®Õn t×m ta gi¸ c¶ ph¶i ch¨ng th«i!", 
			2, "Giao dŞch/yes", 			
			"Nh©n tiÖn ghĞ qua th«i/no")
end;

function yes()
	Sale(13);  				
end;

function no()
end;
