...
function main()
	...
	if (CFG_AnBangHoanMy == 1) then
		tinsert(aryDescribe, "Ta muèn t×m hiÓu vÒ bé An bang Hoµn mü./main_talk");
	end
	if (CFG_TrangBiBachKim == 1) then
		tinsert(aryDescribe, "Ta muèn mêi ng­¬i trïng luyÖn trang bŞ b¹ch kim/platina_main");
		tinsert(aryDescribe, "Ta muèn ®æi thiÕt huyÕt ®¬n/exchange_tiexuedan");
	end
	if (CFG_TrangBiHoangKim == 1) then
		tinsert(aryDescribe, "Nhê ng­¬i gióp ta ph©n m¶nh mét sè trang bŞ hoµng kim./split_entry");
		tinsert(aryDescribe, "Ta muèn sö dông m¶nh trang bŞ ®Ó hîp thµnh trang bŞ Hoµng Kim./compose_entry");
		tinsert(aryDescribe, "Ta muèn ®æi m¹nh vôn/exchange_entry");
	end
