...
function default_talk()
	...
	elseif (nt_getTask(75) == 255) then
		...
		elseif (UTask_tr >= 70*256) and (player_Faction ~= "tianren") then
...
function select()
	...
	elseif (GetSeries() == 3) and (GetCamp() == 0) and (UTask_gb < 5*256) and (UTask_tr < 5*256) then
		if (GetLevel() >= 10) then
