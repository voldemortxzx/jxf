...
function default_talk()
	...
	elseif (nt_getTask(75) == 255) then
		...
		elseif (UTask_sl >= 70*256) and (player_Faction ~= "shaolin") then
...
function enroll_select()
	...
	elseif (GetSeries() == 0) and (GetCamp() == 0) and (UTask_tw < 5*256) and (UTask_sl < 5*256) then
