...
function default_talk()
	...
	elseif (nt_getTask(75) == 255) then
		...
		elseif (UTask_tm >= 70) and (GetFaction() ~= "tangmen") then
...
function select()
	...
	elseif (GetSeries() == 1) and (GetCamp() == 0) and (UTask_wu < 5*256) and (UTask_tm < 5*256) then
		if (GetLevel() >= 10) then
