...
function dlg_offlineaward(player)
	...
	local line1 = format("Rêi m¹ng lÇn tr­íc: <color=red>%s<color><enter>HiÖn ®¨ng nhËp vµo: <color=red>%s<color>",
	...
	local exp = OfflineAward:CalcExp(
						...
						level,
						tran_count)
	local line3 = format("MiÔn phÝ nhËn ®­îc <color=red>%d<color> (kinh nghiÖm)", exp)

	local itr_advtime = player:GetOfflineAdvTime()
	local line4 = format("Ngoµi ra cßn tÝch lòy <color=red>%d<color> phót, cã thÓ sö dông tiªn ®¬n ®Ó nhËn kinh nghiÖm:", itr_advtime)
...
function action_takefreeexp()
	...
	if (OfflineAward:CheckCondition(player) == 0) then
		player:Say("Kh«ng phï hîp ®iÒu kiÖn.")
		return
	end
	local lowtime = player:GetOfflineLowTime()
	if (lowtime <= 0) then
		player:Say("NhËn phÇn th­ëng ñy th¸c miÔn phÝ thµnh c«ng.")
		return
	end
	local exp = OfflineAward:CalcExp(
					AWARDTYPE_FREE,
...
function confirm_usepill()
	...
	if (item_count == 0) then
		Say(format("C¸c h¹ kh«ng trang bÞ <color=red>%s<color>.", inf.Name))
		return
	elseif (advtime < inf.Time) then
		Say(format("Sö dông 1 <color=red>%s<color> cÇn <color=red>%d phót <color> thêi gian ñy th¸c, thêi gian ñy th¸c cña ng­¬i kh«ng ®ñ.",
			inf.Name,
			inf.Time))
		return
	end
	...
	--Ê¹Ó·¾ÅÌìÔÆÓÎµ¤´ÎÊýÉÏÏÞÎª99´Î - Modified by DinhHQ - 20111110 
	AskClientForNumber(inf.CallBack, 1, 99, "Sè lÇn sö dông")
end

function apply_usepill(player, type, count)
--Ê¹Ó·¾ÅÌìÔÆÓÎµ¤´ÎÊýÉÏÏÞÎª99´Î - Modified by DinhHQ - 20111110 
if count > 99 then
	Talk(1, "", "Sè l­îng lín nhÊt lµ 99.")
	return
end
	local inf = PILL_LIST[type]
