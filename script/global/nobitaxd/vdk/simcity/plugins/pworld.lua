...
function SimCityWorld:ATick(tickRate)
	...
	if SimCitizen.UpdateCombatFlagsAll then
		self.combatTickGlobal = (self.combatTickGlobal or 0) + rate
		if (self.combatTickNext == nil) or (self.combatTickNext < self.combatTickGlobal) then
			self.combatTickNext = self.combatTickGlobal + 2 / REFRESH_RATE
			SimCitizen:UpdateCombatFlagsAll()
			if BotAuraKeepAliveAll then BotAuraKeepAliveAll() end   
		end
	end
	
	if SimBotTauntDrain then SimBotTauntDrain(self.combatTickGlobal or 0) end
	for wId, worldInfo in self.data do		
		worldInfo.tick = worldInfo.tick + 1*rate		
		if SimCitizen.UpdateStallFlags and (SimCityWorld:IsThanhThiMap(worldInfo.worldId) == 1 or worldInfo.worldId == 53 or worldInfo.worldId == 20 or worldInfo.worldId == 99 or worldInfo.worldId == 100 or worldInfo.worldId == 101 or worldInfo.worldId == 121 or worldInfo.worldId == 153 or worldInfo.worldId == 174) then
			if (worldInfo.tick_stall == nil or worldInfo.tick_stall < worldInfo.tick) then
				worldInfo.tick_stall = worldInfo.tick + 3/REFRESH_RATE
				SimCitizen:UpdateStallFlags(worldInfo.worldId)
			end
		end

		if worldInfo.showBXH == 1 then			
			if (worldInfo.tick_botladder == nil or worldInfo.tick_botladder < worldInfo.tick) then
				worldInfo.tick_botladder = worldInfo.tick + 3/REFRESH_RATE
				SimCitizen:UpdateBotLadder(worldInfo.worldId)
				if simTK and simTK.checkMarshalFallback then simTK:checkMarshalFallback(worldInfo.worldId) end   
			end
		end
	end
end
