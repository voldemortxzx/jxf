...
function GroupFighter:_randomNgoaiTrang(tbNpc, nNpcIndex)
	local _fg = nil
	_fg = tbNpc.nSettingsIdx  
	local newNgoaiTrang = SimCityNgoaiTrang:doRandom(_fg)
...
function GroupFighter:_checkRank(tbNpc)
	...
	local newRank = 1
	if tbNpc.fightingScore > 1000 then
		newRank = 2
	end
	if tbNpc.fightingScore > 3000 then
		newRank = 3
	end
	if tbNpc.fightingScore > 6000 then
		newRank = 4
	end
	...
	if (tbNpc.rank ~= newRank) then
		...
		tbNpc.rank = newRank
		if SetNpcTitle and tbNpc.finalIndex and tbNpc.finalIndex > 0 then
			SetNpcTitle(tbNpc.finalIndex, newRank)  
		end
	end
		...
		if nNpcIndex > 0 then
			...
			if kind ~= 0 then 
				...
				if nPosCount >= 1 or tbNpc.nSkillId then	
					...
					SetNpcParam(nNpcIndex, 4, 1)   
					SetNpcScript(nNpcIndex, "\\script\\global\\nobitaxd\\vdk\\simcity\\class\\group_fighter.timer.lua")
...
function GroupFighter:_isPlayerEnemyAround(nListId, nNpcIndex)
	...
	if tbNpc.aggroPlayer == nil then
		tbNpc.aggroPlayer = (random(1, 100) <= (SIMBOT_AGGRO_PLAYER_PCT or 30)) and 1 or 0
	end
	if tbNpc.aggroPlayer ~= 1 then return 0 end
	...
	if GetNpcAroundPlayerList then 
	    ...
	    for i = 1, nCount do
			if ((CallPlayerFunction(allNpcs[i], GetFightState) == 1 or SIMBOT_AGGRO_PLAYER == 1) and CallPlayerFunction(allNpcs[i], GetCurCamp) ~= tbNpc.camp) and tbNpc.camp ~= 0 then
				return 1
			end
	    end  
...
function GroupFighter:_joinFight(nListId, reason)
	...
	tbNpc.isFighting = 1
	if SetNpcCombat and tbNpc.finalIndex and tbNpc.finalIndex > 0 then SetNpcCombat(tbNpc.finalIndex, 1, tbNpc.skillCastBua and tbNpc.skillCastBua[1] or 0) end  -- [2026-06-20] co loai xuong-ngua TUC THI luc vao tran (TK qua GroupFighter)
...
function GroupFighter:_joinFightPlayerCheck(nListId, nNpcIndex)
	if (SIMBOT_PROACTIVE_PLAYER or 0) ~= 1 then return 0 end   
		...
		if self:_isPlayerEnemyAround(nListId, nNpcIndex) == 1 then
			...
			if tbNpc.playerID == 0 then
				...
				if worldInfo.showFightingArea == 1 then 
					...
					Msg2Map(tbNpc.nMapId, "<color=white>"..name.."<color> ��nh ng��i t�i "..worldInfo.name.." "..floor(lastPos[1]/8).." "..floor(lastPos[2]/16).."")
				end
			end
...
function GroupFighter:_leaveFight(nListId, isAllDead, reason)
	...
	tbNpc.isFighting = 0
	tbNpc.canSwitchTick = tbNpc.tick + random(tbNpc.tg_nghingoi_minTs or tg_nghingoi.minTs, tbNpc.tg_nghingoi_maxTs or tg_nghingoi.maxTs)	
	self.tbNpcList["n"..nListId] = tbNpc
	reason = reason or "no reason"
	...
	for i=1,N do
		...
		if nNpcIndex > 0 then
			...
			if kind ~= 0 then 
				...
				SetNpcParam(nNpcIndex, 4, 1)   -- [2026-06-25] tag SIM bot (+0x1894=1) cho children -> DoRevive detour bat -> chet DESPAWN, het loop
				if SetNpcLevel then SetNpcLevel(nNpcIndex, 95) end   -- [2026-06-25] ep level 95 (GroupFighter chua bao gio SetNpcLevel -> children m_Level=4 -> cast/skill-render sai, va dong bo voi parent)
				SetNpcScript(nNpcIndex, "\\script\\global\\nobitaxd\\vdk\\simcity\\class\\group_fighter.timer.child.lua")
				SetNpcTimer(nNpcIndex, self.ATICK_TIME)
...
function GroupFighter:ParentTick(nNpcIndex)
	if nNpcIndex > 0 then
		...
		tbNpc._tTick = (tbNpc._tTick or 0) + 1
		if tbNpc._tTick >= 1 then
			tbNpc._tTick = 0
			self:_checkRank(tbNpc)  
			if SetNpcTitle and tbNpc.rank and tbNpc.finalIndex and tbNpc.finalIndex > 0 then
				SetNpcTitle(tbNpc.finalIndex, tbNpc.rank)   
			end
		end
...
function GroupFighter:_doParentTick(nListId)
	...
	then
		...
		if (random(0, tbNpc.attackPlayerChance or SIMBOT_ATTACK_PLAYER_CHANCE or 3) <= 2)  
			then
		...
		for j = 1, maxIndex do	
			...
			if info[3] == "npc" then
				...
				if tbNpc then
					...
					Msg2Map(nW, msg)
					if BotLadderAdd and tbNpc.finalIndex and tbNpc.finalIndex > 0 then BotLadderAdd(swIdx, tbNpc.finalIndex, allPlayers[j][2]) end
				end
			else

				local tbPlayer = SimCityTongKim.playerInTK[nW][info[1]]
				local msg = "<color=white>"..j.." <color=cyan>["..(tbPlayer.phe).."] "..(tbPlayer.rank).." <color>"..(tbPlayer.name).."<color=white> ("..(tbPlayer.score)..")<color>"
				Msg2Map(nW, msg)
