...
settingsPath = "\\settings\\global\\vdk\\simcity\\"
SimCityPlayerNames = {}
SimCityChat = {}
SimCityMap = {}
SimCityPets = {}
...
function loadMap()
    ...
    for i = 1, getn(attractionsData) do
        ...
        if not mapAtractions[worldId] then
            mapAtractions[worldId] = {
                {pX, pY, dialogNpcId, description}
            }
        else
            tinsert(mapAtractions[worldId], {pX, pY, dialogNpcId, description})
        end
    end
    ...
    for i = 1, getn(thanhthiData) do
        ...
        if fileType == "nodes" then
            ...
            -- Stall bot: gom node gan dia danh (isNearAtraction>0) = trung tam thanh (canh tiem/da tau).
            local anodes = {}
            for nodeName, ndata in allNodes do
                if ndata.isNearAtraction and ndata.isNearAtraction > 0 then
                    tinsert(anodes, nodeName)
                end
            end
            ...
            if _dtx then
                for nodeName, ndata in allNodes do
                    if GetDistanceRadius(ndata.x, ndata.y, _dtx, _dty) < 10 then tinsert(dtnodes, nodeName) end
                end
            end
            world.daTauNodes = dtnodes
            world.attractions = mapAtractions[worldId] or {}  
        end
    end
