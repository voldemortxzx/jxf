...
function IsActive(self, simInstance,tbNpc)
    ...
    if GetNpcAroundPlayerList then
        -- Bot luyen cong (train) active tu XA hon (60 o thay vi 32) -> tới gan la da grind san, do delay
        ...
        for i = 1, nCount do
            ...
            -- [SELF-DEFENSE 2026-06-24] bi danh (selfDefTick con hieu luc) -> nham BAT KY player gan (ke ca CUNG camp = ke tan cong), bo qua IsAttackableCamp
            if tbNpc.lastPos and tbNpc.selfDefTick and tbNpc.selfDefTick > tbNpc.tick_breath
                and GetDistanceRadius(tbNpc.lastPos.nX32/32, tbNpc.lastPos.nY32/32, pX, pY) <= scanFightRadius
                and not (GetPlayerPkMode and PIdx2NpcIdx and GetPlayerPkMode(PIdx2NpcIdx(pID)) == 0)
                and (GetNpcLastAttacker and PIdx2NpcIdx and GetNpcLastAttacker(tbNpc.finalIndex) == PIdx2NpcIdx(pID)) then  -- [2026-06-26] CHI nham THU PHAM THAT (player DA danh bot nay); bot-vs-bot tut mau -> thu pham la bot -> KO bu vao player du player o combat-mode
                tbNpc.isPlayerEnemyAround = pID
            end
        end

        if nCount == 0 then
            if tbNpc.mode == "train" or tbNpc.tongkim == 1 then tbNpc.isActive = 1; return 1 end   -- [2026-06-28] train + TK bot LUON active -> danh nhau LIEN TUC du xa player (truoc: TK xa player -> inactive -> MoveInactive march ko danh)
            tbNpc.isActive = 0
            return 0
        end
...
SimMovement.KeoXe = {
    ...
    Move = function(self, simInstance, tbNpc)
        ...
        -- [2026-06-25] KHONG di theo player nua (user: bot cung camp ko bam theo; engage CHI khi BI DANH qua self-def).
        -- Thay bang LANG THANG ngau nhien quanh cho dung -> bot co "viec lam", ko bam, ko dung im frozen.
        ...
        if tbNpc.isFighting == 0 then
            if tbNpc.lastPPosX ~= pX or tbNpc.lastPPosY ~= pY then
                ...
                tbNpc.followMoveUntil = tbNpc.tick_breath + 3*18/REFRESH_RATE   -- player vua di chuyen
            end
            if (tbNpc.followMoveUntil and tbNpc.followMoveUntil > tbNpc.tick_breath) and cachNguoiChoi > DISTANCE_SUPPORT_PLAYER then
            ...
            if (tbNpc.isAttractionAround == 0)then
                ...
                if (tbNpc.CHANCE_ATTACK_NPC and random(1, tbNpc.CHANCE_ATTACK_NPC) <= 2) then
                    ...
                    if countFighting > 0 and tbNpc.worldInfo.showFightingArea == 1 then
                     --   Msg2Map(nW,"C� " .. countFighting .. " nh�n s� �ang ��nh nhau t�i " .. tbNpc.worldInfo.name .. " <color=yellow>" .. floor(myPosX / 8) .. " " .. floor(myPosY / 16) .. "<color>")
                    end
...
--end
        ...
        -- bị nhầm "lag/kẹt" → respawn sau ~vài phút → dwID đổi → SẠP + TIN TỨC RỚT (tìm theo dwID cũ ko thấy).
        -- Bot kẹt thật chỉ đứng yên (vô hại). Nếu cần lại stuck-recovery cho bot DI CHUYỂN, gate theo cờ stall.
        ...
        if node.isExact == 1 then
            NpcRun(tbNpc.finalIndex, nX, nY)
        else
            local targetPos = randomRange({nX, nY}, tbNpc.walkVar or 2)
            NpcRun(tbNpc.finalIndex, targetPos[1], targetPos[2])
        end
...
SimMovement.FormationChild = {
    ...
    Move = function(self, simInstance, tbNpc)
        ...
        if (tbNpc.childrenWalkMode == "random") then
            ...
            if (1) then
                -- gan player + ko fight (ko quai) -> LUON DUNG IM (bo het random) (giu myPos). Bo wander 5% robotic.
            else
        ...
        if targetW > 0 and targetX > 0 and targetY > 0 then
            ...
            -- [2026-06-28] THEO SAU dung im khi player DUNG; chi chay khi player (parent) DI CHUYEN (parent pos doi).
            if (tbNpc.lastParentX ~= pX or tbNpc.lastParentY ~= pY) then tbNpc.lastParentX = pX; tbNpc.lastParentY = pY; tbNpc.followMoveUntil = tbNpc.tick_breath + 3*18/REFRESH_RATE end
            local _pm = (tbNpc.followMoveUntil and tbNpc.followMoveUntil > tbNpc.tick_breath) or (cachNguoiChoi > DISTANCE_FOLLOW_PLAYER)
            if _pm and (targetX ~= myPosX or targetY ~= myPosY) then NpcRun(tbNpc.finalIndex, targetX, targetY) end  -- het robotic 10-con-cung-tick: player dung -> follower dung im
        end
        return 1
    end,
