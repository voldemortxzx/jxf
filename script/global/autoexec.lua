...
if (CFG_NhiemVuLongNgu	== 1) then
	...
	{678,30,5,179,2033,2755,0,"Ninh t­íng qu©n",0,"\\script\\task\\newtask\\master\\zhongli\\enemy_ningjiangjun.lua"},
...
if (CFG_shengdan_jieri2008	== 1) then
	...
	{644,520,1583,3116,[[\script\event\shengdan_jieri\200811\shendanhecheng.lua]],"¤ng Giµ Noel"},
	{644,521,1583,3116,[[\script\event\shengdan_jieri\200811\shendanhecheng.lua]],"¤ng Giµ Noel"},
	{644,522,1583,3116,[[\script\event\shengdan_jieri\200811\shendanhecheng.lua]],"¤ng Giµ Noel"},
	{644,523,1583,3116,[[\script\event\shengdan_jieri\200811\shendanhecheng.lua]],"¤ng Giµ Noel"},
	{644,524,1583,3116,[[\script\event\shengdan_jieri\200811\shendanhecheng.lua]],"¤ng Giµ Noel"},
	{644,525,1583,3116,[[\script\event\shengdan_jieri\200811\shendanhecheng.lua]],"¤ng Giµ Noel"},
	{644,526,1583,3116,[[\script\event\shengdan_jieri\200811\shendanhecheng.lua]],"¤ng Giµ Noel"},	
	};
elseif (CFG_zhongqiu_jieri_2009 == 1) then
	tab_springfestivalNPC = 
	{
...
local tbActNpcList =
{
	...
	--{354,80,1760,3011,	"\\script\\global\\nobitaxd\\npc\\npcsukien.lua","Sø Gi¶ Sù KiÖn"},
	--{354,80,1766,3017,	"\\script\\global\\nobitaxd\\npc\\npcsukien_quanly.lua","Sø Gi¶ Ho¹t §éng"},
...
--ÔÂ¸èµº
	...
	if (CFG_ViemDe	== 1) then
		add_newtasknpc(addnewtasknpc_ViemDe);	-- quest míi Long Ngu Nhiem vu hoang kim
	end
	add_xishancunnpc(xishancunnpc);
	--if (CFG_ChayThuNghiem	== 1) then
	--	add_npc_nobitaxd()		--Npc ADD them nobitaxd
	--end
	add_spreadernpc(spreadernpc);		--¼ÓÔØÓÎÏ·ÍÆ¹ãÔ±
	add_killertasknpc(addkillertasknpc);
	if (CFG_BossSatThu	== 1) then
		add_alltollgatenpc();   			--THEM Boss SAT THU
	end
	add_allpartnernpc();     			--THEM NPC Ban Dong Hanh partner_head.lua
	if (CFG_mid_autumn05	== 1) then
		add_zhongqiunpc();				--THEM NPC add_zhongqiunpc.lua
	end
	if (CFG_zhongqiu2007	== 1) then
		add_zhongqiunpc_2007();				--THEM NPC add_zhongqiunpc.lua
	end
	if (CFG_newyear_2008	== 1) then
		add_newyear_2008();				--THEM NPC add_zhongqiunpc.lua
	end
	if (CFG_jiefang_jieri2009	== 1) then
		add_jiefang_jieri2009();				--THEM NPC add_zhongqiunpc.lua
	end
	if (CFG_jiefang_jieri2008	== 1) then
		add_jiefang_jieri2008();				--THEM NPC add_zhongqiunpc.lua
	end
	cd_addsignnpc()				--THEM NPC cd_addsignnpc.lua
	if (CFG_LienDau	== 1) then
	...
	if (szRegion =="vn") then
		...
		if (CFG_takeboxaward	== 1) then		
			add_dialognpc(addmoontown_mulao);	--THEM NPC Môc L·o autoaddnpc.lua
		end
		if (CFG_mid_autumn06	== 1) then		
			add_dialognpc(add_mid_autumn06);	--THEM NPC Môc L·o autoaddnpc.lua
		end
		if (CFG_birthday_jieri_2009	== 1)	or  (CFG_tagnewplayer	== 1)	then		
			add_dialognpc(add_birthday_jieri_2009);	--THEM NPC ChiÕn T©m T«n Gi¶
		end
		if (CFG_mengjiang2007	== 1) then		
			add_dialognpc(add_mengjiang2007);	--THEM NPC B¸ch HiÓu L·o Nh©n
		end
		if (CFG_xmas2007	== 1) then		
			add_dialognpc(tbxmas2007_girl_binger) ----THEM NPC B¨ng Nhi  \script\event\vnchristmas2007\addnpc.lua
		end
		if (CFG_xmas2006	== 1) then		
			add_dialognpc(tbxmas2006_makesnowman)
		end
		if (CFG_newyear_2008	== 1) then		
