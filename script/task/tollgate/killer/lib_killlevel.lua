...
function add_shashouling(nvalue, series)
	...
	elseif ( nvalue >= 141 ) and ( nvalue<= 160  ) then
		...
		if	(CFG_Xu_Boss_ST9x == 1)	then
			--tbAwardTemplet:GiveAwardByList({{szName = "TiÒn §ång", tbProp = {4,417,1,1,0,0},	nCount = nXu_BossST9x,  nBindState = -2,	nExpiredTime = 10080}}, format("Get %s", "S¸t thñ bÝ b¶o"), 1)
			BossSatThu()
		end
