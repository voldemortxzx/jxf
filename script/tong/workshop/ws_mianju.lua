...
function ws_main(nTongID, nWorkshopID)
	if SVR_CheckUse(nTongID, nWorkshopID, "Tæng qu¶n DÞ dung ph­êng") ~= 1 then
		return 0
	end
	Say("<#>Tæng qu¶n DÞ dung ph­êng: Xem 72 phÐp biÕn hãa cña ta! ChØ cã ng­¬i nghÜ kh«ng tíi chø kh«ng cã viÖc g× ta biÕn kh«ng thµnh.\n C¸c lo¹i mÆt n¹, lo¹i g× còng cã, xem thö cho biÕt!", 3, "Ta muèn xem thö /#use_g_1_ok("..nTongID..","..nWorkshopID..")", 
...
function use_g_1_ok(nTongID, nWorkshopID)
	...
	--	Say("<#>Tæng qu¶n DÞ dung ph­êng: ThËt ®¸ng tiÕc, h«m nay mÆt n¹ ®· ph©n ph¸t hÕt råi, ngµy mai h·y ®Õn vËy!", 0)
	--	return 0;
	--end
...
function SpecialMask(nTongID, nWorkshopID)
	Say("<#>Tæng qu¶n DÞ dung ph­êng: Ng­¬i muèn lµm lo¹i mÆt n¹ nµo?", 4, "MÆt n¹ ngôy trang/#Mask_WeiZhuang("..nTongID..","..nWorkshopID..")",
...
function Mask_WeiZhuang(nTongID, nWorkshopID)
	...
	if (nLevel < WEIZHUANG_LEVEL)then
		Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ ngôy trang sÏ mang l¹i kü n¨ng ngôy trang, gióp cho ng­êi ®eo nã sÏ ngôy trang thµnh h×nh t­îng dÞ dung cña ng­êi ch¬i kh¸c. Nh­ng khi ®¼ng cÊp cña DÞ dung ph­êng ®¹t ®Õn cÊp <color=red>"..WEIZHUANG_LEVEL.."<color> míi cã thÓ chÕ t¸c lo¹i mÆt n¹ nµy, hiÖn tai vÉn ch­a ®ñ cÊp ",
			1, "Lóc kh¸c ta quay l¹i vËy./cancel")
		return
	end
	Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ ngôy trang sÏ mang l¹i kü n¨ng ngôy trang, gióp cho ng­êi ®eo nã sÏ ngôy trang thµnh h×nh t­îng dÞ dung cña ng­êi ch¬i kh¸c. ChÕ t¸c mÆt n¹ ngôy trang cÇn sö dông <color=yellow>"..PRICEMASK_WEIZHUANG.."<color> ®iÓm cèng hiÕn, cã ®ång ý kh«ng?",
...
function GetMask1(nTongID, nWorkshopID)
	if (GetContribution() < PRICEMASK_WEIZHUANG)then
		Say("Tæng qu¶n DÞ dung ph­êng: §iÓm cèng hiÕn kh«ng ®ñ, h·y ra søc v× bang héi ®·.",1,"Kh«ng ®ñ sao/cancel")
		return
	end	
	AddContribution(-PRICEMASK_WEIZHUANG)
	Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ ngôy trang cã thêi gian sö dông lµ 24 giê, cã thÓ ngôy trang ®­îc 10 lÇn.",1,
...
function Mask_Team(nTongID, nWorkshopID)
	...
	if (nLevel < TEAMMASK_LEVEL)then
		Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ tæ ®éi cã thÓ do ®éi tr­ëng hoÆc thµnh viªn trong ®éi sö dông. Sau khi sö dông th× tÊt c¶ c¸c thµnh viªn sÏ gièng hÖt nhau trong mét kho¶ng thêi gian. Cã ®iÒu ®¼ng cÊp DÞ dung ph­êng ®¹t cÊp <color=red>"..TEAMMASK_LEVEL.."<color> míi cã thÓ chÕ t¸c lo¹i mÆt n¹ nµy, hiÖn vÉn ch­a ®ñ cÊp",
			1, "Lóc kh¸c ta quay l¹i vËy./cancel")
		return
	end
	Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ tæ ®éi cã thÓ do ®éi tr­ëng hoÆc thµnh viªn trong ®éi sö dông. Sau khi sö dông th× tÊt c¶ c¸c thµnh viªn sÏ gièng hÖt nhau trong mét kho¶ng thêi gian. ChÕ t¸c lo¹i mÆt n¹ nµy cÇn sö dông mét Ýt ®iÓm cèng hiÕn vµ 1 lo¹i mÆt n¹ bang héi ®Ó lµm mÉu, cã ®ång ý kh«ng?",
...
function Mask_Tong(nTongID, nWorkshopID)
	...
	if (nLevel < TONGMASK_LEVEL)then
		Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ bang héi cã thÓ do bang chñ hoÆc tr­ëng l·o sö dông. Sau khi sö dông th× tÊt c¶ c¸c thµnh viªn sÏ gièng hÖt nhau trong mét kho¶ng thêi gian. Cã ®iÒu ®¼ng cÊp DÞ dung ph­êng ®¹t cÊp <color=red> "..TONGMASK_LEVEL.."<color>míi cã thÓ chÕ t¸c lo¹i mÆt n¹ nµy, hiÖn vÉn ch­a ®ñ cÊp.",
			1, "Lóc kh¸c ta quay l¹i vËy./cancel")
		return
	end
	local nFigure = TONGM_GetFigure(nTongID, GetName())
	if (nFigure ~= TONG_MASTER and nFigure ~= TONG_ELDER)then
		Say("Tæng qu¶n DÞ dung ph­êng: Bang quy chØ cho phÐp bang chñ vµ tr­ëng l·o ®Ó ta chÕ t¸c lo¹i mÆt n¹ nµy, ng­¬i th× kh«ng thÓ chÕ t¸c.", 1, 
			"ThËt lµ tiÕc qu¸/cancel")
		return
	end
	Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ bang héi cã thÓ do bang chñ hoÆc tr­ëng l·o sö dông. Sau khi sö dông th× tÊt c¶ c¸c thµnh viªn sÏ gièng hÖt nhau trong mét kho¶ng thêi gian.  ChÕ t¸c lo¹i mÆt n¹ nµy cÇn sö dông mét Ýt ®iÓm cèng hiÕn vµ 1 lo¹i mÆt n¹ bang héi ®Ó lµm mÉu, cã ®ång ý kh«ng?",
...
function GetMask2(nTongID, nWorkshopID)
	GiveItemUI("ChÕ t¹o mÆt n¹ tæ ®éi","Tæng qu¶n DÞ dung ph­êng: H·y bá vµo mét lo¹i mÆt n¹ th«ng th­êng ®Ó lµm mÉu, chÕ t¹o thµnh c«ng sÏ mÊt ®i vËt mÉu nµy.", "MakeMask2", "onCancel" );
end

--°ï»áÃæ¾ß
function GetMask3(nTongID, nWorkshopID)
	GiveItemUI("ChÕ t¹o mÆt n¹ bang héi","Tæng qu¶n DÞ dung ph­êng: H·y bá vµo mét lo¹i mÆt n¹ th«ng th­êng ®Ó lµm mÉu, chÕ t¹o thµnh c«ng sÏ mÊt ®i vËt mÉu nµy.", "MakeMask3", "onCancel" );
end

--¶ÓÎéÃæ¾ß
function MakeMask2(nCount)		
	if(nCount <= 0) then
		Say("<#>Tæng qu¶n DÞ dung ph­êng: Kh«ng cã nguyªn liÖu kh«ng thÓ chÕ t¸c!", 1, "Ha ha, ®ét nhiªn ta quªn mÊt./cancel")
		return
	end	
	if(nCount > 1) then
		Say("<#>Tæng qu¶n DÞ dung ph­êng: Ng­¬i chØ cÇn bá vµo 1 mÆt n¹ lµ ®­îc råi, bá vµo nhiÒu thÕ ®Ó lµm g×?", 1, "õm, ta biÕt råi/cancel")
		return
	end		
	local itemIdx = GetGiveItemUnit( 1 )
	local g,d,_ = GetItemProp(itemIdx)
	if(g ~= 0 or d ~= 11) then --´íÎóµÀ¾ß
		Say("<#>Tæng qu¶n DÞ dung ph­êng: §©y lµ mÆt n¹ ®ã sao, ta kh«ng hoa m¾t chø?",1,"¤i, ta ®­a nhÇm råi, tr¶ l¹i cho ta./cancel")
		return nil
	end
	local nFeature = GetMaskFeature(itemIdx)
	if (not nFeature) then
		Say("Tæng qu¶n DÞ dung ph­êng: Kh«ng ®­îc råi, mÆt n¹ nµy kh«ng thÓ dïng ®­îc.", 1, "Ta ®æi mét c¸i vËy./cancel")
		return
	end
	local nPrice = aPriceTeamMask[nFeature]
	if (not nPrice)then
		Say("Tæng qu¶n DÞ dung ph­êng: Kh«ng ®­îc råi, kh«ng thÓ dïng mÆt n¹ nµy lµm mÉu ®Ó chÕ t¹o mÆt n¹ tæ ®éi.", 1, "Ta ®æi mét c¸i vËy./cancel")
		return
	end	
	Say("Tæng qu¶n DÞ dung ph­êng: Dïng mÆt n¹ mÉu nµy ®Ó chÕ t¹o mÆt n¹ tæ ®éi cÇn tiªu phÝ <color=yellow>"..nPrice.."<color> ®iÓm cèng hiÕn, nÕu ng­¬i ®ång ý th× ta sÏ ®éng thñ vËy.",
...
function MakeMask3(nCount)		
	...
	if (not nPrice)then
		Say("Tæng qu¶n DÞ dung ph­êng: Kh«ng ®­îc råi, kh«ng thÓ dïng mÆt n¹ nµy lµm mÉu ®Ó chÕ t¹o mÆt n¹ bang héi.", 1, "Ta ®æi mét c¸i vËy./cancel")
		return
	end	
	Say("Tæng qu¶n DÞ dung ph­êng: Dïng mÆt n¹ mÉu nµy ®Ó chÕ t¹o mÆt n¹ tæ ®éi sÏ tiªu phÝ ng©n s¸ch kiÕn thiÕt<color=yellow> "..nPrice.."<color> ®iÓm cèng hiÕn, nÕu ng­¬i ®ång ý th× ta sÏ lµm ®©y!",
...
function TeamMaskMake(nFeature, nPrice, nItemIdx)
	if (GetContribution() < nPrice)then
		Say("Tæng qu¶n DÞ dung ph­êng: Dïng mÆt n¹ nµy lµm mÉu cho mÆt n¹ tæ ®éi cÇn sö dông <color=yellow>"..nPrice.."<color> ®iÓm cèng hiÕn. §iÓm cèng hiÕn cña ng­¬i kh«ng ®ñ, h·y gãp søc cho bang héi tr­íc ®·.",
	...
	Say("Tæng qu¶n DÞ dung ph­êng: MÆt n¹ tæ ®éi cã thÓ gióp cho tÊt c¶ thµnh viªn trong ®éi gièng nhau trong vßng 2 giê.",
		1, "Xin ®a t¹, ta ®i thö ngay ®©y/cancel")		
	local nIdx = AddItem(6,1,1108,1,0,0)
	SetSpecItemParam(nIdx, 1, nFeature)
	SyncItem(nIdx)
	AddContribution(-nPrice)
	Msg2Player("NhËn ®­îc mét mÆt n¹ tæ ®éi")
end
...
function TongMaskMake(nFeature, nPrice, nItemIdx)
	...
	if (TONG_GetBuildFund(nTongID) < nPrice)then
		Say("Tæng qu¶n DÞ dung ph­êng: Dïng mÆt n¹ nµy lµm mÉu cho mÆt n¹ bang héi cÇn sö dông <color=yellow><color=yellow>"..nPrice.."<color> ®iÓm ng©n s¸ch kiÕn thiÕt. Ng©n s¸ch kiÕn thiÕt kh«ng ®ñ!",
			1,"Kh«ng ®ñ sao/cancel")
		return
	end
	--ÔÙ´Î¼ì²éitem
...
function use_buy_2(nTongID, nWorkshopID)
	...
	--end
	local nLevel = TWS_GetUseLevel(nTongID, nWorkshopID)
	local nScale = aLevelContributionCoef[nLevel] --Ö±½ÓÓÃ¹±Ï×¶ÈµÈ¼¶ÏµÊý×÷´òÕÛÏµÊý
...
function BuyCallBack1(nItemIdx, nPrice, nTongID, nWorkshopID)
	...
	local nLevel = TWS_GetUseLevel(nTongID, nWorkshopID)
	--Í­Ç®´òÕÛ»¨·Ñ¹±Ï×¶È=Ãæ¾ßÔ­¼Û*(1-´òÕÛÏµÊý)*1Í­Ç®µÄ¼ÛÖµÁ¿/10000*¹±Ï×¶ÈµÈ¼¶ÏµÊý
...
function USE_G_2(nTongID, nWorkshopID)
	Say("<#>Tæng qu¶n DÞ dung ph­êng: Nh÷ng mÆt n¹ th«ng th­êng do bang ta chÕ t¹o cã thÓ dïng ®iÓm cèng hiÕn ®Ó ®æi.", 2, 
		--"Í­Ç®¹ºÂò/#use_buy_1("..nTongID..","..nWorkshopID..")", 
		"§æi ®iÓm cèng hiÕn lÊy MÆt n¹/#use_buy_2("..nTongID..","..nWorkshopID..")", 
		"Kh«ng mua ®©u/cancel");
end
...
function GET_DESC(nTongID, nWorkshopID, nType)
	...
	local nScale  --µ±Ç°¹ºÂòÃæ¾ßÓÅ»Ý¶È
	...
	if (nUseLevel < wsGetMaxLevel(nTongID, nType))then		
		...
		nNextScale = format("%.1f",aLevelScale[nUseLevel + 1] / 10) --ÏÂ¼¶¹ºÂòÃæ¾ßÓÅ»Ý¶È
