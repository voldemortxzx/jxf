...
function BattleWorld:SignupCost(player, tong, err)
	if (player:GetTiaozhanlingCount() < SIGNUP_TIAOZHANLINGCOUNT or
		...
		err.Msg = format(
			"B¸o danh cÇn <color=red>%d<color> v¹n ng©n l­îng vµ <color=red>%d<color> Khiªu chiÕn lÖnh.",
			SIGNUP_MONEY,
...
function BattleWorld:CheckMapPermission(player, err)
	...
	elseif (player:GetLevel() < 90) then
		msg = "<color=red>Ng­êi ch¬i cÊp 90 trë lªn<color> míi ®­îc tham gia ThÊt Thµnh §¹i ChiÕn."
