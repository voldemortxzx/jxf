...
function kickout()
	...
	for i = 1, getn(players) do
		...
		--local nTL = 200
		local EXP = 300000000
		Msg2Player("<color=green>Hoµn thµnh Th¸ch Thøc Thêi Gian nhËn ®­îc <color=red>"..EXP.." ®iÓm kinh nghiÖm<color>.")
		--AddEnergy(nTL)
		tl_addPlayerExp(EXP)
...
--ID cña Boss V­ît ¶i d?
lo_range_id = {
	{994, 1001},
	{1002, 1005}
};

--ID cña Boss V­ît ¶i kh?
hi_range_id = {
	{1026, 1033},
	{1034, 1037}
};
...
function func_npc_getpos(item, index)
	...
	if GetMissionV(VARV_SOLO_MODE) == 1 then
		local centerX = 1568   -- toa do trung tam map
		local centerY = 3227
		local offsetX = random(-8, 8)
		local offsetY = random(-8, 8)
