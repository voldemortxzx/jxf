...
function want_playboat_socap_todoi()
	...
	if (startboat == 1) then
		if (freeboat == 0) then
			...
			Say(""..strlevel.." Yªu cÇu: <enter>1 v¹n l­îng<enter>1 s¸t thñ gi¶n",2, "Ph¶i! Ta muèn dÉn d¾t ®éi cña ta tham gia./dragon_join_socap_todoi", "Kh«ng cÇn/onCancel")
			return
		end
	else
...
function dragon_join_socap_todoi()
	local blevel = 0
	local havesword = 0
	...
	for i = 1, GetTeamSize() do 
		...
		if (havesword < 1 ) then
			Say("NhiÕp ThÝ ThÇn: H·y chuÈn bÞ 1 s¸t thñ gi¶n råi h·y ®Õn ®©y gÆp ta",0)
