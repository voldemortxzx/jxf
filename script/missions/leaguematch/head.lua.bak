...
function wlls_award_pl_exp(n_rate, n_alevel)
	...
	local n_exp
	if n_rate == 3 then
		-- n_exp = 3000000 -- th¾ng nhËn ®c 3tr Exp
		n_exp = 1000000 -- th¾ng nhËn ®c 3tr Exp
		AddSkillState(451, 1, 0, 2*60*18)
	elseif n_rate == 2 then
		-- n_exp = 1500000 -- hßa nhËn ®c 1.5tr Exp
		n_exp = 800000 -- hßa nhËn ®c 1.5tr Exp
	elseif n_rate == 1 then
		-- n_exp = 1500000 -- thua nhËn ®c 1.5tr Exp
		n_exp = 500000 -- thua nhËn ®c 1.5tr Exp
	end
	wlls_add_exp(n_exp)
		...
		if (n_plidx > 0) then
			...
			if (n_result == 1) then	--Ê¤
				...
				-- if n_level == 2 then --ÎäÁÖÁªÈüÈ¡µÃÊ¤Àû¼Ó»îÔ¾¶È
					...
					-- Msg2Player("Hñy bá ®iÓm n¨ng ®éng.")
				-- end
			elseif (n_result == 0) then	--Æ½
...
function wlls_award_lg(n_level, str_lgname, n_result, n_usedtime)
	...
	for i = 0, n_memcount-1 do
		...
		if (n_plidx > 0) then
			...
			--start
			if (n_result == 1) then	--Ê¤
				-- wlls_award_pl_exp(20 * 4, n_level) -- th¾ng
				wlls_award_pl_exp(3, n_level) -- th¾ng
				wlls_award_pl_Respect(3)
				
				
			elseif (n_result == 0) then	--Æ½
				-- wlls_award_pl_exp(10 * 4, n_level) -- hßa
				wlls_award_pl_exp(2, n_level) -- hßa
				wlls_award_pl_Respect(2)
			else	--¸º
				-- wlls_award_pl_exp(8 * 4, n_level) -- thua
				wlls_award_pl_exp(1, n_level) -- thua
...
function wlls_matchresult(str_league1, str_league2, result, n_usedtime)
	...
	if (not str_league2) then
		...
		if (result == 1) then	--Ê¤
			str = str.." [<color=green>"..str_league1.."<color>] chiÕn th¾ng [<color=green>"..str_league2.."<color>]!"
			wlls_award_lg(n_level, str_league1, 1, n_usedtime)
			wlls_award_lg(n_level, str_league2, 2, n_usedtime)
		else		--Æ½
			str = str.." [<color=green>"..str_league1.."<color>] cïng ®éi [<color=green>"..str_league2.."<color>] hßa nhau!"
...
function wlls_GetAddPoint(nLevel, nWin, nTie)
	return (nWin * 5 * nLevel + nTie * 2 * nLevel) + 5 --  mçi trËn th¾ng sÏ ®c 15 ®iÓm, thua ko c? ®iÓm vµ ®Êu tr­êng s¬ - trung cÊp th¾ng sÏ ®c 10 ®iÓm
end
