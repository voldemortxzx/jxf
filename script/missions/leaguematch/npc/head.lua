...
function wlls_query()
	...
	if (LG_GetLeagueObjByRole(WLLS_LGTYPE, GetName()) ~= 0) then
		...
		tb_option[getn(tb_option)+1] = "Ta muèn kiÓm tra th«ng tin cña m×nh/wlls_query_mylg"
	end
	-- tb_option[getn(tb_option)+1] = "Ta muèn kiÓm tra t×nh h×nh chiÕn ®éi [10 h¹ng ®Çu]/wlls_query_level"
	tb_option[getn(tb_option)+1] = "Ta muèn xem 10 ng­êi ®øng ®Çu/wlls_query_level"
	-- tb_option[getn(tb_option)+1] = "Ta muèn kiÓm tra chØ ®Þnh t×nh h×nh chiÕn ®éi [tªn chiÕn ®éi] /wlls_query_lgnamein"
	-- tb_option[getn(tb_option)+1] = "Ta muèn xem víi tªn chiÕn ®éi/wlls_query_lgnamein"
	tb_option[getn(tb_option)+1] = "Ta muèn xem víi tªn nh©n vËt/wlls_query_plnamein"
...
function wlls_query_level()
	wlls_descript("B¹n muèn kiÓm tra kiÓm tra xÕp h¹ng chiÕn ®éi cÊp nµo?",
		WLLS_LEVEL_DESC[1].." S¬ - Trung cÊp 10 h¹ng ®Çu/#wlls_query_type(1)",
		WLLS_LEVEL_DESC[2].." Cao cÊp 10 h¹ng ®Çu/#wlls_query_type(2)",
		"Trë l¹i/wlls_query",
...
function wlls_query_top10(n_mtype)
	...
	for i = 1, 10 do
		...
		if (not FALSE(n_order)) then
			tb_option[getn(tb_option)+1] = strfill_left("H¹ng "..(-n_order).." - ", 7)..str_lgname.."/#wlls_query_top("..n_ladder..","..i..")"
		end
	end
...
function wlls_query_do(n_lid)
	...
	-------------------------------
		-- LG_ApplySetLeagueTask(WLLS_LGTYPE, str_lgname, WLLS_LGTASK_TOTAL, 49)
	-------------------------------
	...
	local str_rank
	if n_rank == 1 then
		str_rank = "<color=yellow>               XÕp h¹ng<color> <bclr=Blue><color=White>NhÊt<color><bclr> <pic=48>"
	elseif n_rank == 2 then
		str_rank = "<color=yellow>               XÕp h¹ng<color> <bclr=Blue><color=White>Hai<color><bclr> <pic=36>"
	elseif n_rank == 3 then
		str_rank = "<color=yellow>               XÕp h¹ng<color> <bclr=Blue><color=White>Ba<color><bclr> <pic=56>"
	elseif n_rank >= 4 then
		str_rank = "<color=yellow>               XÕp h¹ng thø<color> <color=green>"..n_rank.."<color>"
	else
		str_rank = "<color=red>VÉn ch­a xÕp h¹ng<color>"
	end

	local n_width = 17
	local str = " "..str_mtype.."<enter>"
	str = str .. strfill_center("[<color=yellow>"..str_lgname.."<color>]", 53+21, "-")
	-- str = str .. strfill_left("<enter>gåm cã<color=yellow>"..n_memcount.."<color> Thµnh viªn", n_width+21).."<color=cyan>"..str_mem.."<color><enter>"
	str = str .. strfill_left("<enter>®· tham gia thi ®Êu: <color=yellow>"..n_total.."<color> trËn", n_width+21).."<enter>th¾ng lîi<color=yellow> "..n_win.." <color>trËn / hßa<color=green> "..n_tie.." <color>trËn / thÊt b¹i<color=red> "..n_lose.." <color>trËn<enter>"
	str = str .. strfill_left("",n_width).."Tû lÖ th¾ng:<color=yellow> "..strsub(f_rate,1,4).."<color>%<enter>"
	str = str .. strfill_left("®iÓm th­ëng: <color=yellow>"..n_point.."<color>", n_width+21).."thêi gian chiÕn ®Êu:<color=yellow> "..n_time.."<color> gi©y<enter>"
	str = str .. "<enter>"..strfill_center(""..str_rank.."", 53+21).."<color>"
	wlls_descript(str)
end
...
function wlls_descript(str, ...)
	...
	str = "<link=image[0,8]:\\spr\\npcres\\passerby\\passerby092\\passerby092_st.spr><link>"..str
...
function wlls_add_option(tb, str)
	...
	if (n_type == n_next) then
		...
		tb[getn(tb)+1] = "Ta muèn xem hç trî thi ®Êu/#wlls_help("..n_type..","..n_level..",1)"
	else
