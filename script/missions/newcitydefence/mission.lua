...
function JoinMission(RoleIndex, Camp)
	...
	-- Thi?t l?p danh hi?u m?c ??nh (Binh s?)
	local campnum = 89
	if Camp == 2 then
		campnum = 94
	end
	Title_AddTitle(campnum, 0, 9999999)
	Title_ActiveTitle(campnum)
	AddSkillState(RANK_SKILL, 0, 0, 999999)
...
function OnLeave(RoleIndex)
	...
	-- Xoa danh hi?u
	Title_ActiveTitle(0)
	AddSkillState(RANK_SKILL,0,0,0)
	
	-- ??a ng??i ch?i v? map ch? khi out game ho?c r?i kh?i mission
